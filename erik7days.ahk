...
Gui, Show , w550 h520, erik's 7days scripts v03
...
Gui, Add, Hotkey, x400 y90 vHKacceptquest

Gui, Add, Text, x280 y158 Left,spam misc hotkey
Gui, Add, Hotkey, x400 y158 vHKspammisc
Gui, Add, Text, x280 y133 Left,key to spam
Gui, Add, Hotkey, x400 y130 vTEXTspammisc
...
loadKeys()
{
	...
	IniRead, HKacceptquest, erik7days.ini, HotKeys, HKacceptquest, ""
	IniRead, HKspammisc, erik7days.ini, HotKeys, HKspammisc, ""
	IniRead, TEXTspammisc, erik7days.ini, HotKeys, TEXTspammisc, ""
	...
	GuiControl,, HKacceptquest, % HKacceptquest
	GuiControl,, HKspammisc, % HKspammisc
	GuiControl,, TEXTspammisc, % TEXTspammisc
...
saveKeys()
{
	...
	IniWrite, %HKacceptquest%, erik7days.ini, HotKeys, HKacceptquest
	IniWrite, %HKspammisc%, erik7days.ini, HotKeys, HKspammisc
	IniWrite, %TEXTspammisc%, erik7days.ini, HotKeys, TEXTspammisc
...
applyKeys()
{
	...
	if (HKacceptquest != ""){
		HotKey,%HKacceptquest%,acceptQuest
	}
	if (HKspammisc != ""){
		HotKey,%HKspammisc%,spamMisc
	}
...
Toggle := !Toggle 
While Toggle {
	ControlClick, , 7 Days To Die, , LEFT, 1, , ,
	rollShort()
	If !Toggle
		break
 }
...
Toggle := !Toggle
If (Toggle){
	ControlClick, , 7 Days To Die, , LEFT, 1, D, ,
} else {
	ControlClick, , 7 Days To Die, , LEFT, 1, U, ,
}
return
...
Toggle := !Toggle 
While Toggle {
	ControlClick, , 7 Days To Die, , RIGHT, 1, , ,
	rollShort()
	If !Toggle
		break
 }
...
Toggle := !Toggle
If (Toggle){
	ControlClick, , 7 Days To Die, , RIGHT, 1, D, ,
} else {
	ControlClick, , 7 Days To Die, , RIGHT, 1, U, ,
}
return
...
acceptQuest:
{
	IfWinActive, 7 Days To Die
	{
		Toggle := !Toggle 
		While Toggle{
				PixelSearch, cyanX, cyanY, 0, (A_ScreenHeight / 2.5), A_ScreenWidth, A_ScreenHeight, 0x24A18D, 10, RGB Fast
				if ErrorLevel{
					MsgBox, no more quests
					Toggle := !Toggle
					break
				}
				MouseMove, cyanX, cyanY, 10
				Send {Click Down}
				rollShort()
				Send {Click Up}
				rollShort()
				...
				ImageSearch, imageX, imageY, 0, (A_ScreenHeight / 2), A_ScreenWidth, A_ScreenHeight, *10 accept.png
				MouseMove, imageX, imageY, 10
